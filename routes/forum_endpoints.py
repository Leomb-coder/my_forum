...
from controllers.replies_controller import get_replies_by_thread_id, create_reply
...
def thread(forum_slug, thread_id, thread_slug):
    try:
        ...
        replies = get_replies_by_thread_id(thread_id)

        return render_template(
            'thread.html',
            thread=current_thread,
            get_user=get_user_by_id,
            get_forum=get_forum_by_id,
            user=get_user_by_id(session['user_id']),
            replies=replies
        )
...
@forum_bp.route('/<int:thread_id>/create_reply', methods=['POST'])
def create_reply_route(thread_id):
    try:
        data = request.get_json()
        user_id = session['user_id']
        content = data.get('reply-content')

        created_reply = create_reply(thread_id, user_id, content)
